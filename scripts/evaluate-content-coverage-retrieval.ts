...
import { evaluateRetrievalReview, type RetrievalReviewFile } from "../lib/content-coverage/retrieval-evaluation";
import { COVERAGE_TOPIC_SPECS } from "../lib/content-coverage/topic-specs";
import { TREATMENT_DETAILS } from "../lib/treatments";

const reviewPath = path.resolve(process.cwd(), process.argv[2] ?? ".tmp/content-coverage-retrieval-review.json");

async function main() {
  const file = JSON.parse(await readFile(reviewPath, "utf8")) as RetrievalReviewFile;
  ...
  console.log(JSON.stringify(evaluateRetrievalReview(file, lexical), null, 2));
}
