...
| `ai_agent_briefings` | `id` | AI 운영 브리핑 스냅샷 | Supabase Admin 전용 |
| `ai_agent_suggestions` | `id` | AI 운영 제안 | Supabase Admin 전용 |
| `ai_agent_actions` | `id` | AI 운영 승인/반려/적용 로그 | Supabase Admin 전용 |
| `ai_agent_jobs` | `id` | AI 운영실 제안 생성 잡 상태 | 서버 쓰기 + 인증 사용자 읽기(Realtime) |
| `ai_agent_job_events` | `id` | AI 운영실 제안 생성 진행 이벤트 | 서버 쓰기 + 인증 사용자 읽기(Realtime) |
| `ai_agent_outcomes` | `id` | AI 운영실 관측 신호 기록 | Supabase Admin 전용 |
...
| `ai_agent_jobs` | 인증 사용자 | 차단 | 관리자 UI Realtime 구독용, 쓰기는 service_role 전용 |
| `ai_agent_job_events` | 인증 사용자 | 차단 | 관리자 UI Realtime 구독용, 쓰기는 service_role 전용 |
| 나머지 | 차단 | 차단 | 기본 전체 차단 |
...
AI 운영실 스키마: `supabase/migrations/004_ai_ops.sql`, `supabase/migrations/005_ai_ops_jobs.sql`, `supabase/migrations/006_ai_ops_low_traffic.sql`.
