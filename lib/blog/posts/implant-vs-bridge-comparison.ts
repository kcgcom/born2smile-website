    ...
    content: [
      {
        ...
        content:
          "치아를 하나 잃으면 그 자리만 비는 것이 아니에요. 책꽂이에서 책 한 권을 빼면 나머지가 옆으로 쓰러지듯, 양옆 치아가 빈 공간으로 기울어지고 맞물리는 반대쪽 치아가 솟아오르면서 교합(물림)이 무너집니다. 씹는 힘이 남은 치아에 불균등하게 전달되어 추가 손상 위험이 커지고, 잇몸뼈(치조골)도 자극이 사라진 부위부터 서서히 흡수돼요. 오래 방치할수록 이후 치료가 복잡해지므로, 가능한 한 빨리 수복 계획을 세우는 것이 좋습니다.",
      },
      {
