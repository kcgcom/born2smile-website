...
const promoteSchema = z.object({ actionKey: z.string().min(3).max(300) });
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const state = await saveActionReviewState(parsed.data.actionKey, { status: parsed.data.status, notes: parsed.data.notes }, auth.email);
...
export async function POST(request: NextRequest) {
  ...
  try {
    return Response.json({ data: await promoteActionToPlanner(parsed.data.actionKey, auth.email) }, { status: 201, headers: HEADERS });
  } catch (error) {
    const message = knownError(error);
    if (message) return Response.json({ error: "ACTION_ERROR", message }, { status: 400, headers: HEADERS });
    Sentry.captureException(error);
