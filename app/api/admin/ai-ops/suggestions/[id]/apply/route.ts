...
import { applyAiSuggestion } from "@/lib/admin-ai-ops";
...
export async function POST(
  ...
  if (!parsed.success) {
    return Response.json(
      { error: "VALIDATION_ERROR", message: "반영 요청 형식이 올바르지 않습니다", issues: parsed.error.issues },
      { status: 400, headers: HEADERS },
    );
  }
  ...
  try {
    const data = isAiOpsRemoteEnabled()
      ? (await proxyAiOpsJson<AiOpsSuggestionListItem>({
          path: `/ai-ops/suggestions/${suggestionId}/apply`,
          method: "POST",
      ...
      : await applyAiSuggestion({ id: suggestionId, actorEmail: auth.email, note: parsed.data.note });

    return Response.json({ data }, { headers: HEADERS });
  } catch (error) {
    Sentry.captureException(error);
    return Response.json(
      { error: "API_ERROR", message: error instanceof Error ? error.message : "제안을 반영할 수 없습니다" },
