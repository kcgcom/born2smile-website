...
│   ├── /treatments/prosthetics (틀니 및 심미보철)
│   ├── /treatments/pediatric (소아치료)
│   ├── /treatments/restorative (보존치료)
│   └── /treatments/scaling (스케일링)
├── /blog (블로그)
│   ├── 공지사항
│   └── 치과 건강 정보 칼럼
└── /contact (예약/상담)
...
| 3 | **진료 분야** | 아이콘 카드 그리드 (임플란트, 교정, 심미보철, 소아, 보존, 스케일링) |
...
- 진료시간: 요일별 진료시간 테이블
- 비급여 수가표: 의료법 의무 공개 항목 (블랑쉬의 `/price.php` 참고)
- 초진 안내: 첫 방문 시 안내 플로우
...
### 4.4 블로그 (`/blog`)
- 공지사항 (휴진 안내, 이벤트 등)
- 치과 건강 정보 칼럼 (치아 관리법, 시술 정보 등)
- MDX 기반으로 쉽게 글 작성 가능한 구조
