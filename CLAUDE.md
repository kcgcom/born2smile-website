...
**기본 톤**: 친절하고 따뜻하면서도 전문성을 잃지 않는 동네 치과 원장님 톤. 환자에게 직접 설명하듯 쓰되, 가볍지 않게.

**문체 원칙:**
- 기본 어미는 `~이에요`, `~해요`, `~거든요`, `~드려요`, `~해 주세요` 등 부드러운 존댓말 사용
- 같은 문단 안에서 `~합니다`체와 `~해요`체가 섞여도 되지만, 갑자기 딱딱해지거나 갑자기 가벼워지지 않도록 흐름 유지
- 정보 전달 문장은 `~합니다`/`~됩니다` 허용, 공감·권유 문장은 `~해요`/`~세요` 선호

**피해야 할 표현:**
- 의태어·의성어 남용: ~~슬금슬금~~, ~~반짝반짝~~ → `조금씩`, `깨끗하게` 등 담백한 표현 사용
- 마케팅 느낌 표현: ~~투자~~, ~~혁신적인~~, ~~획기적인~~ → `방법`, `효과적인` 등 의료 맥락에 맞는 표현 사용
- 영어 전문 용어 단독 사용: ~~리테이너~~, ~~Essix 타입~~ → 한국어 명칭 사용 (유지장치, 고정식/가철식)
- 교과서적 나열: ~~A, B, C가 영향을 미칩니다~~ → `A나 B, C도 여기에 영향을 줍니다`처럼 자연스럽게 연결

**권장 패턴:**
- 환자 공감 → 전문 설명 → 실천 안내 순서로 전개
- 섹션 도입부에서 환자가 궁금해할 만한 상황이나 질문으로 시작
- 전문 용어는 한국어 명칭을 기본으로, 필요 시 괄호 안에 보충 설명 (영어 용어가 아닌 쉬운 설명)
- 마지막 섹션에서 서울본치과의 따뜻한 마무리 멘트 포함
